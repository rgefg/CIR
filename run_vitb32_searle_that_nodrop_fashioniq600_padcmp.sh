...
mkdir -p "${LOG_DIR}/fashioniq_nopad" "${LOG_DIR}/fashioniq_pad" /tmp/fashioniq600_padcmp
...
run_fashion_block() {
  ...
  "${PYTHON_BIN}" data/merge_lora_ties.py \
  ...
  CUDA_VISIBLE_DEVICES="${EVAL_GPU}" "${PYTHON_BIN}" src/eval_retrieval.py \
    --model "ViT-B/32" \
    --gpu 0 \
    --batch-size 32 \
    --img2text-arch phi \
