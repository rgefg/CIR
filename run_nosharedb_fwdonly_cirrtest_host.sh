...
LOG="${ROOT}/logs/DistillCIR_ParallelDualLoRA_BS56_Accum8_ViTL14_SEARLEPhi_And_NoDrop_NoSharedB_FwdOnly_CIRR"
CKPT="${LOG}/checkpoints"
OUT="${LOG}/cirr_test_step1400"
MERGED="/tmp/DistillCIR_ParallelDualLoRA_BS56_Accum8_ViTL14_SEARLEPhi_And_NoDrop_NoSharedB_FwdOnly_CIRR_step1400_merged.pt"
...
CUDA_VISIBLE_DEVICES=3 "${PY}" "${ROOT}/src/eval_retrieval.py" \
  --resume "${MERGED}" \
  --openai-pretrained \
  --model "ViT-L/14" \
  --eval-mode cirr_test \
