...
- Best local merged results observed so far:
  - FashionIQ:
    - `dress R@10 = 24.24` (`that`-aligned, `Hybrid`, `step 600`)
    - `shirt R@10 = 30.91` (`No-drop`, `raw base + ema geo`, `step 800`)
    - `toptee R@10 = 31.00` (`that`-aligned, `TIES`, `step 800`)
  - GeneCIS:
    - `focus_attribute R@1 = 20.75` (`ViT-L-shaped ckpt`, `dropout=0.5`, merged `step 1400`)
    - `change_attribute R@1 = 17.05` (`ViT-L-shaped ckpt`, `dropout=0.5`, merged `step 800`)
    - `focus_object R@1 = 14.85` (`ViT-L-shaped ckpt`, `dropout=0.5`, merged `step 800`)
    - `change_object R@1 = 17.04` (`No-drop`, `raw base + ema geo`, `step 800`)
...
- Image-root A/B, same best merge weight, `target-pad` on:
  - local original images vs `images_upscaled`
  - `dress`: local `R@10/R@50 = 24.05 / 46.11`, upscaled `23.70 / 46.06`
  - `shirt`: local `29.98 / 48.28`, upscaled `30.23 / 47.79`
  - `toptee`: local `30.80 / 51.71`, upscaled `30.75 / 51.35`
  - Conclusion:
    - Super-res FashionIQ images do not give a meaningful gain on this best merged checkpoint.
    - It is essentially flat, with tiny trade-offs across categories, so image upscaling is not a strong direction for now.

- `ViT-L/14 + SEARLE Phi`, `no-drop`, merged `step 600`, `FashionIQ`:
  - `dress`: `R@1/R@5/R@10/R@50/R@100 = 7.34 / 17.45 / 23.95 / 46.85 / 57.91`
  - `shirt`: `12.27 / 24.53 / 30.62 / 50.29 / 59.42`
  - `toptee`: `11.22 / 25.24 / 32.53 / 53.09 / 63.34`
  - Mean:
    - `avg R@10 = 29.03`
    - `avg R@50 = 50.08`
  - Conclusion:
    - `SEARLE Phi` on `ViT-L/14` gives a strong `toptee` gain and improves the overall `FashionIQ` average.
    - It does not beat the older best single-category `dress`, but it is one of the strongest balanced `FashionIQ` merged results we have.
