...
- Current judgment:
  ...
  - Eval-time `composed + text` feature mixing is not a good drop-in on the current best merged checkpoint.
    - Checkpoint: `ThatAlign step800`, `raw base + geo ema`, `TIES`
    - Old merged FashionIQ `R@10`: `dress/shirt/toptee = 24.00 / 29.88 / 31.00`
    - With eval-time `0.5 * composed + 0.5 * text`: `20.18 / 26.94 / 28.00`
    - Conclusion: direct feature fusion hurts all three categories on the current best merge and should not be used as the new default.
