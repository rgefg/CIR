...
GPU="${1:-5}"
OUT_JSONL="$RUN_DIR/genecis_step1400_ablation.jsonl"
...
run_eval() {
  local layers="$1"
  local merge_mode="$2"
  local merged_ckpt="/tmp/${RUN_NAME}_genecis_l${layers}.pt"
  local out_json="/tmp/${RUN_NAME}_genecis_l${layers}.json"
  local merge_args=(
