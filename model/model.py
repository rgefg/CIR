...
class ReasoningProjector(nn.Module):
    """
    Map a compact CLIP reasoning latent into a short sequence of soft prompts
    for a frozen decoder-only LLM.

    The `stem` intentionally mirrors IM2TEXT so we can warm-start it from a
    trained pic2word checkpoint without tying the retrieval branch weights.
    """
    ...
    def __init__(
        ...
        self.stem = IM2TEXT(
            ...
            output_dim=clip_token_dim,
            n_layer=n_layer,
            dropout=dropout,
        )
        self.norm = nn.LayerNorm(clip_token_dim)
        self.head = nn.Linear(clip_token_dim, prompt_length * llm_hidden_size)

    def init_from_img2text(self, img2text: nn.Module):
        """
        Copy matching pic2word weights into the projector stem as a warm-start.
        """
        src = img2text.module if hasattr(img2text, "module") else img2text
        if not isinstance(src, IM2TEXT):
            raise TypeError(f"Expected IM2TEXT for warm-start, got {type(src)}")
        msg = self.stem.load_state_dict(src.state_dict(), strict=False)
        return msg

    def forward(self, z_reason: torch.Tensor):
        x = self.stem(z_reason)
        x = self.norm(x)
        x = self.head(x)
        return x.view(z_reason.size(0), self.prompt_length, self.llm_hidden_size)
    ...
    def encode_text_img_vis(self, text, img_tokens, split_ind=4):
        ...
        for i, sample in enumerate(x):
            ...
            if isinstance(img_tokens, tuple):
                ...
                # img_tokens is expected to be [B, d_model]; insert the *current sample's* token
                # (do NOT reshape the whole batch into a single long vector).
...
def _apply_lora_to_linear_layers(module: nn.Module, r: int, alpha: int, dropout: float = 0.0,
                                 name_filter=None):
    """
    Recursively replace nn.Linear with LoRALinear.
    name_filter: optional list of substrings; only apply if any matches the full module path.
    """
    for name, child in list(module.named_children()):
        ...
        if isinstance(child, nn.MultiheadAttention):
            if (name_filter is None) or any(k in full_name for k in name_filter):
                setattr(module, name, LoRAMultiheadAttention(child, r=r, alpha=alpha, dropout=dropout))
        elif isinstance(child, nn.Linear):
            if (name_filter is None) or any(k in full_name for k in name_filter):
                setattr(module, name, LoRALinear(child, r=r, alpha=alpha, dropout=dropout))
        else:
            _apply_lora_to_linear_layers(child, r=r, alpha=alpha, dropout=dropout, name_filter=name_filter)


def enable_lora_on_clip(clip: nn.Module, r: int = 64, alpha: int = 16, dropout: float = 0.0):
    """
    Apply LoRA to all nn.Linear modules in CLIP.
    Freeze all CLIP params except LoRA A/B and logit_scale.
    """
    # 1) apply LoRA wrappers
    _apply_lora_to_linear_layers(clip, r=r, alpha=alpha, dropout=dropout)

    # 2) freeze everything
    for n, p in clip.named_parameters():
        p.requires_grad = False

    # 3) unfreeze LoRA params
    for n, p in clip.named_parameters():
        if n.endswith(".A") or n.endswith(".B"):
            p.requires_grad = True

    # 4) allow logit_scale
    if hasattr(clip, "logit_scale"):
        clip.logit_scale.requires_grad = True
